    ...
    @forelse ($siswa as $item)
        <div class="card mb-3 TranskTugas">
            ...
            <div class="tengah">
                ...
                <div class="kanan">
                    @if ($item->jawabanTugas->isNotEmpty())
                        {{-- Ambil id_mapel dari jawaban tugas pertama --}}
                        @php
                            $id_mapel = $item->jawabanTugas->first()->tugas->mapel->id_mapel ?? null;
                        @endphp
                        @if ($id_mapel)
                            <a href="/trans/nilai/{{ $item->id_murid }}/{{ $id_mapel }}/lihat" class="btn btn-info transkip-btn">Detail</a>
                        @endif
                    @endif
                </div>
        </div>
